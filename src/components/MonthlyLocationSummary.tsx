...
interface MonthlyLocationSummaryProps {
  incidents: Incident[];
  onClose?: () => void;
  startDate?: string;
  endDate?: string;
...
export function MonthlyLocationSummary({ incidents, onClose, startDate, endDate }: MonthlyLocationSummaryProps) {
  // Get top 6 locations based on incident count
  ...
  const monthlyData = useMemo(() => {
    ...
    return months.map(month => {
      ...
      // Count incidents for each top location in this month
        ...
        <div className="space-y-1">
          {payload.map((entry: any, index: number) => (
            <div key={entry.name} className="flex items-center justify-between gap-4">
              ...
              <span className="text-white">{entry.value} chamados</span>
            </div>
          ))}
          <div className="pt-2 mt-2 border-t border-gray-700">
            <div className="flex items-center justify-between">
              ...
              <span className="text-white font-bold">
                {payload.reduce((sum: number, entry: any) => sum + entry.value, 0)} chamados
              </span>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="bg-[#151B2B] p-6 rounded-lg space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {topLocations.map((location, index) => {
          const totalIncidents = monthlyData.reduce((sum, month) => sum + (month[location] || 0), 0);
