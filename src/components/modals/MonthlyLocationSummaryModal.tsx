...
// MonthlyLocationSummaryModal
//
// - Exibe o sumário mensal dos indicadores por localidade.
...
interface MonthlyLocationSummaryModalProps {
  onClose: () => void;
}

export function MonthlyLocationSummaryModal({ onClose }: MonthlyLocationSummaryModalProps) {
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-[#151B2B] rounded-lg p-8 max-w-xl w-full relative shadow-2xl border border-gray-700">
        ...
        <h2 className="text-2xl font-bold text-white mb-6 flex items-center gap-2">
          ...
          Sumarização Mensal por Localidade
        </h2>
        <div className="flex items-center justify-center h-64 text-gray-400">
          {/* Placeholder para gráfico ou conteúdo */}
          Gráfico/Indicador de Sumarização Mensal por Localidade aqui
        </div>
      </div>
    </div>
  );
